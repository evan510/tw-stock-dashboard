...
menu = st.sidebar.radio(
    "短線作戰導航：",
    [
        "🚀 1. 今日短線即時作戰室 (夜盤+09:30雷達)",
        "🎯 2. 多因子強勢狙擊池 (萬里無雲/買黑/投信)",
        "🩺 3. 個股 AI 深度診斷室 🤖[Gemini AI]",
        "📡 4. 社群情報與名師風向 🤖[Gemini AI]",
        "💼 5. 我的持股庫存管家 (停損警報+Line推播)"
    ],
...
# ================= 模組 1：今日短線即時作戰室 (夜盤+09:30雷達) =================
if menu == "🚀 1. 今日短線即時作戰室 (夜盤+09:30雷達)":
    st.title("🚀 今日短線即時作戰室 (夜盤風向 ＋ 09:30 早盤爆量起漲)")
    st.caption("短線當沖/隔日沖必備第一站：【夜盤美股跳空試算】+【早盤起漲爆量雷達】+【盤中題材利多催化】")
...
# ================= 模組 3：個股 AI 深度診斷室 🤖[Gemini AI] =================
# ================= 模組 2：多因子強勢狙擊池 (萬里無雲/買黑/投信) =================
elif menu == "🎯 2. 多因子強勢狙擊池 (萬里無雲/買黑/投信)":
    st.markdown("""
    <div style="display:flex; align-items:center; gap:10px; margin-bottom:4px;">
        <h1 style="margin:0; padding:0; display:inline-block;">🎯 多因子強勢狙擊池</h1>
        <span class="pill pill-buy" style="font-size:0.85rem; padding:4px 10px;">⚡ 量價 + 均線 + 籌碼 三合一強勢篩選</span>
    </div>
    """, unsafe_allow_html=True)
    st.caption("短線操盤核心軍火庫：結合【大盤短線環境評分】+【成交熱門爆量榜】+【老王萬里無雲飆股/買黑不買紅】+【投信鎖碼波段榜】+【自選與題材庫】")
    ...
    pool_tab1, pool_tab2, pool_tab3, pool_tab4 = st.tabs([
        "⚡ 分頁 A：動態成交熱門榜 (Top 30 + RS突破)",
        "👑 分頁 B：老王均線戰法 (萬里無雲 / 買黑不買紅)",
        "🎯 分頁 C：投信鎖碼波段榜 (Top 30 + 精選Top 5)",
        "💎 分頁 D：自選追蹤池 ＆ 精選題材庫"
    ])
    ...
    with pool_tab2:
        st.subheader("👑 老王均線操盤金律專區")
        st.caption("源自《oldwangstock 老王愛說笑》操盤天條：萬里無雲、買黑不買紅、破線無條件停損！")
        
        sub_ow1, sub_ow2 = st.tabs(["🚀 萬里無雲飆股池", "🎯 買黑不買紅精選"])
        with sub_ow1:
    ...
    with pool_tab3:
        st.subheader("🎯 投信鎖碼波段選股榜 (持股 1~2 週)")
                ...
                with cols[i]:
                    ...
                    with st.expander("入選理由"):
                        for sig in s['signals']:
                            st.write(f"• {sig}")
            st.markdown("---")
            st.markdown("#### 📋 投信買超鎖碼評分總榜單 (Top 30)")
...
elif menu == "🩺 3. 個股 AI 深度診斷室 🤖[Gemini AI]":
    st.markdown("""
    <div style="display:flex; align-items:center; gap:10px; margin-bottom:4px;">
        <h1 style="margin:0; padding:0; display:inline-block;">🩺 個股 AI 深度診斷室</h1>
        <span class="pill pill-blue" style="font-size:0.85rem; padding:4px 10px;">🤖 Google Gemini API 按需進階診斷</span>
        <span class="pill pill-gold" style="font-size:0.85rem; padding:4px 10px;">⚡ 當日快取保護 (0 Token 浪費)</span>
    </div>
    """, unsafe_allow_html=True)
    st.caption("全方位 AI 操盤量化體檢：融合【趨勢均線】+【量價結構】+【RS大盤相對強度】+【老王防守紀律】+【互動K線圖】+【Gemini 首席操盤手深度覆盤】")
...
{ai_narr}
            ...
            st.markdown("---")
            # 5. 互動 K 線圖室 (台股 紅漲 / 綠跌)
            st.markdown(f"#### 📈 【{data['name']} ({data['symbol']})】日 K 線與均線布林系統")
            df_k = get_stock_history(data['symbol'], period='4mo')
            if not df_k.empty:
                fig = go.Figure()
                ...
                fig.add_trace(go.Scatter(x=df_k.index, y=df_k['5MA'], line=dict(color='#f59e0b', width=1.5), name='5MA'))
                fig.add_trace(go.Scatter(x=df_k.index, y=df_k['10MA'], line=dict(color='#a855f7', width=1.5), name='10MA'))
                fig.add_trace(go.Scatter(x=df_k.index, y=df_k['20MA'], line=dict(color='#3b82f6', width=2.2), name='20MA(生命線)'))
                fig.add_trace(go.Scatter(x=df_k.index, y=df_k['60MA'], line=dict(color='#10b981', width=1.5), name='60MA(季線)'))
                fig.add_trace(go.Scatter(x=df_k.index, y=df_k['BB_Upper'], line=dict(color='gray', width=1, dash='dash'), name='布林上軌'))
                fig.add_trace(go.Scatter(x=df_k.index, y=df_k['BB_Lower'], line=dict(color='gray', width=1, dash='dash'), name='布林下軌'))
                fig.update_layout(
...
# ================= 模組 4：社群情報與名師風向 🤖[Gemini AI] =================
    ...
    st.markdown("""
    <div style="display:flex; align-items:center; gap:10px; margin-bottom:4px;">
        <h1 style="margin:0; padding:0; display:inline-block;">📡 社群情報與名師風向</h1>
        <span class="pill pill-blue" style="font-size:0.85rem; padding:4px 10px;">🤖 Google Gemini API 深度賦能</span>
        <span class="pill pill-gold" style="font-size:0.85rem; padding:4px 10px;">⚡ 本地 JSON 智慧快取保護 (0 Token 浪費)</span>
    </div>
    """, unsafe_allow_html=True)
    st.caption("匯聚【名師 YouTube 7天逐字影音解構】與【股市同學會 5天散戶情緒溫度計】，由 Google Gemini AI 深入提煉核心論點與多空心理！")

    guru_tab1, guru_tab2, guru_tab3, guru_tab4 = st.tabs([
        "🔥 哲哲 (郭哲榮) 最新 7 天影音深度解構",
